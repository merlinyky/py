...
def calculate_columns(df, calculations_config):
    """
    Perform column-wise calculations as specified in the configuration.
    """
    for calc in calculations_config:
        if calc['operation'] == '+':
            df[calc['new_column']] = df[calc['columns'][0]] + df[calc['columns'][1]]
        elif calc['operation'] == '-':
            df[calc['new_column']] = df[calc['columns'][0]] - df[calc['columns'][1]]
        elif calc['operation'] == '*':
            df[calc['new_column']] = df[calc['columns'][0]] * df[calc['columns'][1]]
            ...
            df[calc['new_column']] = df[calc['columns'][0]] / df[calc['columns'][1]]
        elif calc['operation'] == 'diff':
            df[calc['new_column']] = df[calc['columns'][0]].diff()
        elif calc['operation'] == 'pct_change':
